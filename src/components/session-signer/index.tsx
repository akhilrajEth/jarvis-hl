...
interface SessionSignerProps {
  embedded_wallet: WalletWithMetadata;
}

export default function SessionSignerCard({ embedded_wallet }: SessionSignerProps) {
  ...
  const hasSessionSigners = embedded_wallet.delegated === true;
  ...
  const handleClientSign = useCallback(async () => {
    ...
    try {
      const message = `Signing this message to verify ownership of ${embedded_wallet.address}`;
      let signature;
  ...
  return (
    <div className="flex flex-col gap-4 p-4 border border-gray-200 rounded-lg">
      <div className="text-sm text-violet-700">
        {embedded_wallet.walletClientType === "privy" ? "Embedded " : ""}Wallet:{" "}
        {embedded_wallet.address.slice(0, 6)}...
        {embedded_wallet.address.slice(-4)}
      </div>

      <div className="flex flex-col sm:flex-row gap-2">
        <button
          onClick={() => addSessionSigner(embedded_wallet.address)}
        ...
        <button
          onClick={() => removeSessionSigner(embedded_wallet.address)}
          disabled={isLoading || !hasSessionSigners}
          className={`text-sm py-2 px-4 rounded-md text-white ${
            isLoading || !hasSessionSigners
